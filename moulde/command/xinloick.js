...
module.exports.handleEvent = async function({ api, args, event, client	}) {
  ...
  if(indexOf("xin lỗi ck iu") || indexOf("xin lỗi ck yêu") || indexOf("xin lỗi vợ ck") || indexOf("xin lỗi vợ ck")) {
    var mention = Object.keys(event.mentions)[0];
    if(!mention) return api.sendMessage("Tag người bạn muốn xin lỗi", event.threadID);
  var emoji = ["♥️","❤️","💛","💚","💙","💜","🖤","💖","💝","💓","💘","💍","🎁","💋","💎","💠","🌈","🌍","🌕","☀️","💑","💞","💗"];
  var random_emoji = emoji[Math.floor(Math.random() * emoji.length)];
  ...
  var arraytag = [];
    ...
    a("Anh Đừng Giận Vợ Nữa Mà 🥺🥺");
    await delay(2000);
    a("Xin Vui Lòng Đợi 5p Để Anh Yêu Hết Giận Rồi Nhắn ☘️☘️☘️");
    await delay(2000);
    a("Tha Lỗi Cho Vợ Nha Anh Ơi 🥺😭");
    await delay(2000);
    a({body: name_1+" "+"💓"+" "+name_2, mentions: arraytag, attachment: imglove});
    fs.unlinkSync(__dirname+'/cache/love.gif');
    fs.unlinkSync(__dirname+'/cache/avt.png');
